...
export default function page() {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      const response = await axios.get(
        `https://4786-160-202-38-214.ngrok-free.app/webscraping/api/v1/web-scraping/?search=${searchTerm}`,
        {
          headers: {
            accept: "*/*",
          },
  ...
  const saveData = async () => {
    if (data.length != 0) {
      const url = `https://4786-160-202-38-214.ngrok-free.app/webscraping/api/v1/web-scraping/?fileName=${searchTerm}`;
  ...
  return (
    <>
      <div className="text-white h-screen flex flex-col items-center px-2 bg-[#343541]">
        <h1 className="text-5xl font-bold mb-10 mt-20">Web Scraping</h1>
        {/* search */}
